...
for nomean in range(1):
    ...
    def random_average():                                                          #多次随机分布，取平均值
        ...
        for i in range(av_times):
            Uxy=Uxy_list2[i]
            remake1_R += backhaul_remake1(l, U_sum, S_sum, B, Uxy, C)
            remake2_R += backhaul_remake2(l, U_sum, S_sum, B, Uxy, C)
        average_R1=remake1_R/av_times
        average_R2=remake2_R/av_times
    ...
    for i in range(x_times):
        ...
        y2.append(temp[1])
        U_sum += 5
    mp.plot(x,y1,'o:',color='black',label='纯策略纳什均衡方案',linewidth=1)
    mp.plot(x,y2,'s-',color='black',label='改进策略纳什均衡方案',linewidth=1)
    mp.xlim((5,50))
    mp.ylim((0,10))
    mp.xticks(np.arange(5,55,5))
    mp.yticks(np.arange(0,11,1))
    mp.legend()
    mp.xlabel('N/个')
    mp.ylabel('迭代次数/次')
    ...
    mp.savefig("..\..\仿真报告\批注修改图-衰落\无人机数目变量-迭代次数.png", dpi=750, bbox_inches = 'tight')
