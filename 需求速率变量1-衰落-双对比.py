...
for nomean in range(1):
    ...
    def random_average():                                                          #多次随机分布，取平均值
        ...
        for i in range(av_times):
            ...
            normal_R_extra += sum(backhaul_normal_extra(l, U_sum, S_sum, B, Uxy, C))
            remake1_R_extra += sum(backhaul_remake1_extra(l, U_sum, S_sum, B, Uxy, C))
            remake1_R_recycle_extra += sum(backhaul_remake1_recycle_extra(l, U_sum, S_sum, B, Uxy, C))
            remake2_R_extra += sum(backhaul_remake2_extra(l, U_sum, S_sum, B, Uxy, C))
    ...
    for i in range(x_times):
        ...
        C=np.array(C)+1e+6/8
    y1=np.array(y1)/U_sum
    y2=np.array(y2)/U_sum
    y2_recycle=np.array(y2_recycle)/U_sum
    y3=np.array(y3)/U_sum
    y1_extra=np.array(y1_extra)/U_sum
    y2_extra=np.array(y2_extra)/U_sum
    y2_recycle_extra=np.array(y2_recycle_extra)/U_sum
    y3_extra=np.array(y3_extra)/U_sum
    ...
    mp.savefig(f"..\..\仿真报告\批注修改图-衰落\需求速率变量-平均有效回程传输速率2.png", dpi=300, bbox_inches = 'tight')
