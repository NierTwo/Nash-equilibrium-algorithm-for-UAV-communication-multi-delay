...
for nomean in range(1):
    ...
    def backhaul_remake2_extra(l,U_sum,S_sum,B,Uxy,C):                                    ###l,U_sum,S_sum,S_sum,B为自定义的常数,Uxy,C为数组
        B1xy=[0,0]
        Uxy=sorted(Uxy,key=lambda x:min([np.linalg.norm([x[0]-B1xy[0],x[1]-B1xy[1]])]),reverse=True)
        ...
        def MLoss(x, y):
            mloss = Plos(x, y) * Loss1(x, y) + (1 - Plos(x, y)) * Loss2(x, y)
            return mloss
    ...
    def backhaul_remake1_recycle_extra(l,U_sum,S_sum,B,Uxy,C):
        B1xy=[0,0]
        B2xy=[0,l]
        B3xy=[l,0]
        B4xy=[l,l]
    ...
    def backhaul_remake1_extra(l,U_sum,S_sum,B,Uxy,C):
        B1xy=[0,0]
        B2xy=[0,l]
        B3xy=[l,0]
        B4xy=[l,l]
    ...
    def backhaul_normal_extra(l,U_sum,S_sum,B,Uxy,C):
        B1xy=[0,0]
        B2xy=[0,l]
        B3xy=[l,0]
        B4xy=[l,l]
    ...
    def random_average():                                                          #多次随机分布，取平均值
        ...
        normal_R_extra = 0
        remake1_R_extra = 0
        remake1_R_recycle_extra = 0
        remake2_R_extra = 0
        for i in range(av_times):
            Uxy=Uxy_list[i]
            normal_R += min(backhaul_normal(l, U_sum, S_sum, B, Uxy, C))
            remake1_R += min(backhaul_remake1(l, U_sum, S_sum, B, Uxy, C))
            remake1_R_recycle += min(backhaul_remake1_recycle(l, U_sum, S_sum, B, Uxy, C))
            remake2_R += min(backhaul_remake2(l, U_sum, S_sum, B, Uxy, C))
            normal_R_extra += min(backhaul_normal_extra(l, U_sum, S_sum, B, Uxy, C))
            remake1_R_extra += min(backhaul_remake1_extra(l, U_sum, S_sum, B, Uxy, C))
            remake1_R_recycle_extra += min(backhaul_remake1_recycle_extra(l, U_sum, S_sum, B, Uxy, C))
            remake2_R_extra += min(backhaul_remake2_extra(l, U_sum, S_sum, B, Uxy, C))
        ...
        average_R1_extra=normal_R_extra/av_times
        average_R2_extra=remake1_R_extra/av_times
        average_R2_recycle_extra=remake1_R_recycle_extra/av_times
        average_R3_extra=remake2_R_extra/av_times
        ...
        average_list.append(average_R1_extra)
        average_list.append(average_R2_extra)
        average_list.append(average_R2_recycle_extra)
        average_list.append(average_R3_extra)
        return average_list
    ...
    U_sum=15
    C=[1e+6/8 for i in range(U_sum)]
    x_times=6
    av_times=100
    Uxy_list = [random_distribution(l, U_sum) for i in range(av_times)]
    ...
    for i in range(x_times):
        x.append(C[0]/1000000*8)
        temp=random_average()                                  #取av_times次平均
        y1.append(temp[0]*8/1000000)
        y2.append(temp[1]*8/1000000)
        y2_recycle.append(temp[2]*8/1000000)
        y3.append(temp[3]*8/1000000)
        y1_extra.append(temp[4]*8/1000000)
        y2_extra.append(temp[5]*8/1000000)
        y2_recycle_extra.append(temp[6]*8/1000000)
        y3_extra.append(temp[7]*8/1000000)
        C=np.array(C)+1e+6/8
    y1=np.array(y1)
    y2=np.array(y2)
    y2_recycle=np.array(y2_recycle)
    y3=np.array(y3)
    y1_extra=np.array(y1_extra)
    y2_extra=np.array(y2_extra)
    y2_recycle_extra=np.array(y2_recycle_extra)
    y3_extra=np.array(y3_extra)
    mp.figure(dpi=100, figsize=(10, 8))
    mp.plot(x,y1,'^:',color='black',label='Direct transmission with fading channel',linewidth=1,markerfacecolor='none')
    mp.plot(x,y2,'D-.',color='black',label='Pure NE strategy with fading channel',linewidth=1,markerfacecolor='none')
    mp.plot(x,y2_recycle,'2--',color='black',label='Pure NE strategy with resource recovery with fading channel',linewidth=1,markerfacecolor='none')
    mp.plot(x,y3,'*-',color='black',label='Proposed improved NE strategy with fading channel',linewidth=1,markerfacecolor='none')
    mp.plot(x,y1_extra,'o:',color='black',label='Direct transmission without fading channel',linewidth=1,markerfacecolor='none')
    mp.plot(x,y2_extra,'+-.',color='black',label='Pure NE strategy without fading channel',linewidth=1,markerfacecolor='none')
    mp.plot(x,y2_recycle_extra,'x--',color='black',label='Pure NE strategy with resource recovery without fading channel',linewidth=1,markerfacecolor='none')
    mp.plot(x,y3_extra,'s-',color='black',label='Proposed improved NE strategy without fading channel',linewidth=1,markerfacecolor='none')
    mp.xlim((1,6))
    mp.ylim((0,3))
    mp.xticks(np.arange(1,7,1))
    mp.yticks(np.arange(0,3.5,0.5))
    mp.legend()
    mp.xlabel(r'$\Lambda$/Mbps')
    mp.ylabel('Rmin/Mbps')
    # backhaul_normal(l,U_sum,S_sum,B,Uxy,C)
    # backhaul_remake1(l,U_sum,S_sum,B,Uxy,C)
    # backhaul_remake2(l,U_sum,S_sum,B,Uxy,C)
    mp.savefig(f"..\..\仿真报告\批注修改图-衰落\需求速率变量-最小有效回程传输速率2.png", dpi=300, bbox_inches = 'tight')
