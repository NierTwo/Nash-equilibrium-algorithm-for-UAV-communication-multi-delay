...
for nomean in range(1):
    ...
    y3=np.array(y3)
    mp.plot(x,y1,'o:',color='black',label='Direct transmission',linewidth=1)
    mp.plot(x,y2,'+-.',color='black',label='Pure NE strategy',linewidth=1)
    mp.plot(x,y2_recycle,'x--',color='black',label='Pure NE strategy with resource-recover',linewidth=1)
    mp.plot(x,y3,'s-',color='black',label='Proposed improved NE strategy',linewidth=1)
    mp.xlim((20,30))
    mp.ylim((0,9))
    ...
    mp.savefig(f"..\..\仿真报告\批注修改图-衰落\功率变量-最小有效回程速率.png", dpi=750, bbox_inches = 'tight')
